...
def start(update, context):
    ...
    tglog.save()
    if log['state'] == 0:
        update.message.reply_html(TEXTS['START'], reply_markup=inline_btns("lang"))
    else:
        if log['state'] >= 4:
            log.clear()
            log['state'] = 4
            update.message.reply_html(TEXTS['MENU1'][tg_user.lang], reply_markup=btns('menu', lang=tg_user.lang))
        else:
            log['state'] = 0
            update.message.reply_html(TEXTS['START'], reply_markup=inline_btns("lang"))
...
def callback_handler(update, context, kwargs=None):
    ...
    if state == 0:
        ...
        if data == "uz":
            ...
            query.message.reply_html(TEXTS['START'], reply_markup=inline_btns("lang"))
            return 0
        query.message.reply_text(TEXTS['NAME'][tg_user.lang])
        tglog.messages = log
        tglog.save()
        return 0
